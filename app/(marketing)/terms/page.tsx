...
export default function TermsPage({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  const document = (
    <div className={embedded ? 'text-[#0F172A]' : 'rounded-3xl border border-white/60 bg-white/45 backdrop-blur-xl shadow-[0_18px_50px_rgba(15,23,42,0.08)] p-6 sm:p-10'}>
      <h1 className={embedded ? 'text-2xl font-bold text-slate-900 mb-2' : 'text-4xl font-bold text-slate-900 mb-3'}>
        {t.title}
      </h1>
      ...
      <p className="text-slate-700 mb-4 text-sm sm:text-base">{t.languageNote}</p>
      <p className="text-slate-700 mb-8 leading-relaxed text-sm sm:text-base">
        {t.preamble}
      </p>

      {t.sections.map((section) => (
        <section key={section.id} className="mb-8">
          <h2 className={embedded ? 'text-lg font-semibold text-slate-900 mt-4 mb-2' : 'text-2xl font-semibold text-slate-900 mt-6 mb-3'}>
            {section.title}
          </h2>
          {section.description && (
