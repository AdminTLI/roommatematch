...
--
-- ╔══════════════════════════════════════════════════════════════════════════════╗
-- ║  🚨 NEVER APPLY THIS FILE TO ANY DATABASE  🚨                              ║
-- ║                                                                              ║
-- ║  This file is a DEVELOPMENT REFERENCE ONLY.  It contains STUB functions     ║
-- ║  with hardcoded scores (similarity_score=0.8, schedule_overlap=0.7, …).     ║
-- ║  Applying it will OVERWRITE the production algorithm and cause ALL           ║
-- ║  compatibility scores to cluster at fixed values, breaking matching for      ║
-- ║  every user on the platform.                                                ║
-- ║                                                                              ║
-- ║  Production implementation (authoritative):                                  ║
-- ║    supabase/migrations/                                                      ║
-- ║      202603181200_yp_dual_marketplace_compute_compatibility_score.sql        ║
-- ║      20260601230000_fix_get_user_matches_name_column.sql                     ║
-- ║                                                                              ║
-- ║  If you need to update the algorithm, add a new timestamped migration to     ║
-- ║  supabase/migrations/ — do NOT edit this file.                               ║
-- ╚══════════════════════════════════════════════════════════════════════════════╝
--
-- INTENTIONAL GUARD: the DO block below raises an exception if this file is
-- accidentally executed, preventing silent score corruption.
DO $$
BEGIN
  RAISE EXCEPTION
    'ABORT: db/functions/matching_engine.sql is a placeholder and must NEVER be applied to a database. '
    'Use supabase/migrations/ for all schema changes. '
    'See the file header for the authoritative migration paths.';
END;
$$;
