-- Migration: Fix smoking dealbreaker logic
-- Issue: Current logic is too strict - it fails when one user wants no smoking rule (M8_Q8=true) 
--        and the other doesn't want the rule (M8_Q8=false), even if the second user isn't a smoker.
-- Solution: Only fail if one user wants no smoking AND the other user is actually a smoker.
--           We check user_housing_preferences.smoking_preference to determine if user is a smoker.
--           If smoking_preference is 'smoking_ok', we infer the user might be a smoker or is okay with smoking.

-- Fix check_hard_constraints to use actual smoking status
...
BEGIN
  ...
  v_a_wants_no_smoking := extract_boolean_value(v_a_prefs->'M8_Q8') = true;
  v_b_wants_no_smoking := extract_boolean_value(v_b_prefs->'M8_Q8') = true;
  ...
  -- Note: SELECT INTO will set variable to NULL if no row is found (no error raised)
  -- This is safe and doesn't require exception handling
  SELECT smoking_preference INTO v_a_smoking_pref 
  FROM user_housing_preferences WHERE user_id = user_a_id;
  
  SELECT smoking_preference INTO v_b_smoking_pref 
  FROM user_housing_preferences WHERE user_id = user_b_id;
  ...
  IF v_a_smoking_pref IS NOT NULL AND v_b_smoking_pref IS NOT NULL THEN
    ...
    END IF;
  ELSE
    -- Fallback: If smoking_preference is not set, use the old logic for backward compatibility
    -- This handles cases where user_housing_preferences might not be set yet
    -- Only fail if one user wants no smoking and the other explicitly doesn't want the rule
    -- This is more lenient than before - we don't fail just because one doesn't want the strict rule
    -- We only fail if there's an explicit conflict (one wants no smoking, other allows it)
    -- But since we can't determine if the other user is actually a smoker, we'll be more lenient
    -- and only fail if BOTH users have conflicting preferences AND we can infer a conflict
    
    -- Actually, let's be even more conservative: if we can't determine smoking status,
    -- don't fail on smoking conflicts. The user_housing_preferences should be set during onboarding.
