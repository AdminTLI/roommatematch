-- Migration: Fix resolve_user_preferences and get_dimension_value to use public. prefixes
-- Date: 2025-11-27
-- Description: These functions have SET search_path = '' but don't use public. prefixes
--              for table names and function calls, causing them to fail.

-- Fix get_dimension_value
CREATE OR REPLACE FUNCTION public.get_dimension_value(
  p_user_id UUID,
  p_question_key TEXT,
...
BEGIN
  ...
  IF v_m8_q8_value IS NOT NULL AND public.extract_boolean_value(v_m8_q8_value) = true THEN
  ...
  IF v_m8_q14_value IS NOT NULL THEN
    ...
    v_has_pet_tolerance := 
      (v_m8_q15_value IS NOT NULL AND public.extract_boolean_value(v_m8_q15_value) = true) OR
      (v_m8_q16_value IS NOT NULL AND public.extract_boolean_value(v_m8_q16_value) = true) OR
      (v_m8_q17_value IS NOT NULL AND public.extract_boolean_value(v_m8_q17_value) = true);
...
-- Grant permissions
GRANT EXECUTE ON FUNCTION public.get_dimension_value(UUID, TEXT, JSONB) TO authenticated;
