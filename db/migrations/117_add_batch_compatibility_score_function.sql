-- Migration: Add batch compatibility score function
-- This function computes compatibility scores for multiple user pairs in a single call
...
BEGIN
  ...
  LOOP
    ...
    -- This leverages the existing optimized function
    ...
    FROM compute_compatibility_score(user_a_id, user_b_id_item) cs;
  END LOOP;
...
COMMENT ON FUNCTION compute_compatibility_scores_batch IS 
  'Computes compatibility scores for multiple user pairs in a single call. Returns results with user_b_id included for easy mapping.';
