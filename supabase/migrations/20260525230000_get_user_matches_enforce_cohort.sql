-- Security/correctness fix: enforce same-cohort (student ↔ student, professional ↔ professional)
-- filtering inside get_user_matches.
--
-- Without this filter a professional user calling /api/matches/my received student profiles
-- as match candidates — a cross-cohort PII exposure. The daily cron (run_daily_matching)
-- already filters by user_type; this patch aligns the on-demand RPC.
...
BEGIN
  ...
  WITH filtered_users AS (
    ...
    WHERE p.user_id != p_user_id
      ...
      AND p.user_type = v_caller_user_type
...
-- Keep permissions consistent with the security-hardening migrations
-- (202604301700 / 202604301800): only service_role may call this function.
