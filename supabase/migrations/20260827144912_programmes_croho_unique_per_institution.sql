...
-- Legacy programs table (user_academic FK target): same rule scoped by university
...
COMMENT ON INDEX public.idx_programs_university_croho_unique IS
  'CROHO is unique per university offering, not globally';

-- rio_code can also repeat across institutions when derived from SKDB opleiding codes
